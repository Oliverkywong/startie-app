...
export class TeamService {
  ...
  async createTeam(
    ...
    try {
      const event = await this.knex<Team>("team")
        .select("*")
        .where("name", name);
      if (event.length > 0) {
  ...
  async getAllTeams(
    ...
    const count = await this.knex.raw(query, params);
  ...
  async getTeamForAdmin(id: number) {
    const team = await this.knex<Team>("team as t")
      .select(
        ...
        "t.searchcategory_id as category_id",
        "t.description as description",
        "t.shortDescription as shortDescription",
        "t.profilepic as profilepic",
        "s.name as status",
        "sc.name as category",
        "t.clickrate as clickrate",
        this.knex.raw("ARRAY_AGG(distinct u.username) AS users"),
        this.knex.raw("ARRAY_AGG(distinct tag.name) AS tags")
      )
  ...
  // edit team
  // -------------------------------------------------------------------------------------------------------------------
  async updateTeam(id: number, input: TeamListInput) {
    const teamInfo = await this.knex<Team>("team")
      .update({
        name: input.name,
